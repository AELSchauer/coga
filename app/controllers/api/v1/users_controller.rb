class Api::V1::UsersController < ApplicationController
  ...
  def create
    ...
    # else
      ...
      if !@user.valid?
        ...
        render status: 400, json: { data: { message: "Not a company email" } }
      else
        ...
        if @user.save
          ...
          render status: 201, json: { data: @user.response_attributes }
        else
          render status: 500, json: { data: { message: "Unable to save user" } }
        end
      end
    # end
  end
  ...
  def options
    params.require(:data).require(:options).permit(:company_id, :partner_id, :role_name)
  end

  def user_params
    params.require(:data).require(:user).permit(:username, :primary_email, :password, :first_name, :last_name)
  end
end
